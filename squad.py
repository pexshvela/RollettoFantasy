...
async def search_player_results(message, state: FSMContext):
    ...
    # Guard against non-text messages (stickers/photos) — message.text is None.
    if not message.text or not message.text.strip():
        await message.answer("🔍 Please type a player name to search.")
        return
    ...
    await message.answer(
        "🔍 Results for <b>" + _esc(message.text) + "</b>:",
        parse_mode="HTML", reply_markup=kb.as_markup()
    )
...
async def swap_do(callback: CallbackQuery, state: FSMContext):
    ...
    captain   = (user or {}).get("captain", "")
    formation = (user or {}).get("formation", "4-3-3")
    # If this swap pushes the captain to the bench, the ×2 would be wasted
    # (only starters are scored). Keep the armband in the starting XI by moving
    # it to the player who takes the captain's starting spot.
    captain_moved = False
    if captain and captain == starter_pid:
        captain = sub_pid
        captain_moved = True
        await sheets.update_user(uid, captain=sub_pid)
    snapshot = dict(squad)
    snapshot["captain"]   = captain
    snapshot["formation"] = formation
    ...
    kb.button(text=t(lang, "back_home"), callback_data="home:back")
    kb.adjust(1)
    swap_text = t(lang, "swap_done", out=sub_name, in_=starter_name)
    if captain_moved:
        swap_text += "\n\n" + t(lang, "captain_moved", name=sub_name)
    await callback.message.edit_text(
        swap_text,
        parse_mode="HTML",
        reply_markup=kb.as_markup()
